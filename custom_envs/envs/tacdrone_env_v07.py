...
class TacDroneHoverEnvV07(gym.Env):
    """
    ...
    def _compute_reward(self, action_normed: np.ndarray) -> tuple[float, dict[str, float]]:
        ...
        yaw = _wrap_pi(self._yaw())
        action_delta = action_normed - self.last_action

        reward_terms = {
            "alive": 1.0,
            "pos": float(-2.0 * np.dot(pos_err, pos_err)),
            "vel": float(-0.25 * np.dot(vel, vel)),
            "tilt": float(-0.5 * tilt**2),
            "yaw": float(-0.1 * yaw**2),
            "ang": float(-0.05 * np.dot(gyro, gyro)),
            "act_delta": float(-0.02 * np.dot(action_delta, action_delta)),
        }
