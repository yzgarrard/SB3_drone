...
class TacDroneHoverEnvV06(gym.Env):
    """
    ...
    def step(self, action: np.ndarray):
        action = (np.clip(action, -1, 1) + 1.0) / 2.0  # rescale to [0, 1]
        action = -0.74182095 + 9.48418091*action + 2.05334713*action**2

        for _ in range(self.frame_skip):
            # self.data.ctrl[:] = motor_force_cmd
            self.data.ctrl[:] = self.alpha*self.data.ctrl[:] + (1-self.alpha)*action
        ...
        if terminated:
            reward_terms["termination"] = -100.0  # large penalty for crashing/going out of bounds
